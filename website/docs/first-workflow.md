...
**This doc is for first-time users.** If you already have the framework set up, this is your canonical onboarding workflow. For tool-specific setup only, see [`QUICKSTART.md`](quickstart). For installation paths, see [`INSTALL.md`](install).
...
> **Note:** `npm run validate:all` validates the framework itself using the current source-of-truth gate set from `package.json`. It is separate from your app's tests.
...
| Read the quickstart for your tool | [QUICKSTART.md](quickstart) |
| View adapter docs | [adapters/README.md](adapters/README.md) |
