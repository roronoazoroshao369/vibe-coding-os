#!/usr/bin/env node
// tests/skills/safe-refactor.test.mjs
// v2.15.0 — Wave B Deliverable 3: Regression test for safe-refactor skill
...
const SKILL_PATH = resolve(__dirname, '../../skills/core/safe-refactor/SKILL.md');
...
if (existsSync(SKILL_PATH)) {
  ...
  check('Mentions refactor', new RegExp('refactor', 'i').test(content));
  check('Mentions behavior preservation', new RegExp('behavior|preserv', 'i').test(content));
  check('Mentions tests', new RegExp('test', 'i').test(content));
} else {
