...
Use when you need to plan a safe optional memory provider adapter. Use the smallest relevant memory skill and avoid duplicating existing `project-memory`, `context-retrieval`, `privacy-filter`, `session-summarizer`, or `agent-handoff` behavior.
