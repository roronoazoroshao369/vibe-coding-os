...
Use when you need to review candidate memory for secrets and sensitive data. Use the smallest relevant memory skill and avoid duplicating existing `project-memory`, `context-retrieval`, `privacy-filter`, `session-summarizer`, or `agent-handoff` behavior.
