...
if (cmd === 'create') console.log(JSON.stringify(await createTask(store, { title: flags.title, owner: flags.owner, dependsOn: flags.dependsOn ? String(flags.dependsOn).split(',') : [] }), null, 2));
else if (cmd === 'list') console.log(JSON.stringify(await listTasks(store), null, 2));
else if (cmd === 'status') console.log(JSON.stringify(await updateTaskStatus(store, flags.id, flags.to), null, 2));
...
else { console.error('usage: runtime-task <create|list|status|next|import> [--title --owner --dependsOn --id --to --file]'); process.exit(1); }
