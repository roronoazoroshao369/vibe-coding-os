...
| 4 | [supermemoryai/supermemory](https://github.com/supermemoryai/supermemory) | MIT | inspiration | Graph relationship typing + isLatest, Memory-vs-RAG, MemScore triple, hybrid search tuning | `skills/memory/memory-ingestion/`, `skills/memory/memory-search/`, `skills/memory/memory-search/`, `references/features/memory-vs-rag.md` | `references/sources/supermemoryai-supermemory.md` |
| 5 | [thedotmack/claude-mem](https://github.com/thedotmack/claude-mem) | Apache-2.0 | inspiration | Project/worktree scoping, periodic digest, env-sanitizer checklist, hook event taxonomy | `skills/memory/project-memory/`, `skills/memory/session-capture/`, `skills/memory/project-memory/`, `adapters/hooks/memory-hooks-contract.md` | `references/sources/thedotmack-claude-mem.md` |
| 6 | [yeachan-heo/oh-my-claudecode](https://github.com/yeachan-heo/oh-my-claudecode) | MIT | adapted | Commit decision-trailers, context-budget audit, skillify-from-session, instinct-extraction, agent role routing | `skills/core/git-guardrails/`, `skills/meta/context-budget/`, `skills/memory/session-capture/`, `skills/meta/context-budget/`, `skills/agents/*` | `references/sources/yeachan-heo-oh-my-claudecode.md` |
| 7 | [affaan-m/ECC](https://github.com/affaan-m/ECC) | MIT | adapted | Continuous-learning/instinct concept, context-budget heuristics | `skills/meta/context-budget/`, `skills/meta/context-budget/` | `references/sources/affaan-m-ecc.md` |
| 8 | [multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills) | ⚠️ MIT-declared-incomplete | inspiration (re-write only) | 4 engineering-discipline principles: Think-Before-Coding, Simplicity-First, Surgical-Changes, Goal-Driven-Execution (viết lại nguyên bản, KHÔNG copy) | `skills/prompts/karpathy-engineering-discipline/`, `skills/prompts/anti-overengineering/`, `skills/core/goal-driven-execution/`, `skills/core/verification-before-done/` | `references/sources/multica-ai-andrej-karpathy-skills.md` |
...
> **Cách đọc cột “File local chính”:** đường dẫn trong bảng là targets của synthesis runtime local. Một số đã được implement trong repo, một số có thể là target planned/future khi runtime mở rộng. Nếu file tồn tại, vẫn phải hiểu là **local original implementation**, không phải file upstream được vendored hay port nguyên trạng.

| # | Upstream repo | License | Trạng thái | Ý tưởng synthesis (adapted, code gốc) | File local chính |
|---|---|---|---|---|---|
| 20 | [eyaltoledano/claude-task-master](https://github.com/eyaltoledano/claude-task-master) | MIT + Commons Clause | inspiration | Task status convention + dependency-aware next-task selection → local JSON store | `runtime/tasks/task-store.mjs`, `scripts/runtime-task.mjs` (implemented local original files if present; not upstream runtime) |
...
> **Foundation (original):** `runtime/core/*.mjs` (ids, atomic JSON fs-store, lock files, events.jsonl, schemaVersion validation, privacy redaction), `templates/runtime-config-template.json` — actual where present, planned where absent.
> **MVP commands:** `runtime:init|validate|task|memory|checkpoint|team|session` trong `package.json` — actual where present, planned/future where absent.
