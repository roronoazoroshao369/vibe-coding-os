# Skill: Privacy Exclusion

## Purpose

Exclude secrets and sensitive data before memory storage or use. This skill adapts `thedotmack/claude-mem` concepts into Vibe Coding OS while composing with existing memory skills such as `project-memory`, `session-summarizer`, `context-retrieval`, `memory-search`, `memory-retrieval`, `memory-privacy`, `privacy-filter`, and `agent-handoff`.
