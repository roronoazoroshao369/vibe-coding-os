...
Inject scoped, safe, citation-backed memory into the active task. This skill adapts `thedotmack/claude-mem` concepts into Vibe Coding OS while composing with existing memory skills such as `project-memory`, `session-summarizer`, `context-retrieval`, `memory-search`, `memory-retrieval`, `memory-privacy`, `privacy-filter`, and `agent-handoff`.
